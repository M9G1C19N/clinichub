...
import {
    ...
    ClipboardList, User, SlidersHorizontal, X,
...
// Today's queue client-side search
const queueSearch = ref('')
const filteredTodayQueue = computed(() => {
    const s = queueSearch.value.toLowerCase().trim()
    if (!s) return props.todayQueue ?? []
    return (props.todayQueue ?? []).filter(item =>
...
<template>
    <AppLayout title="Nurse Intake">
        ...
        <div v-if="activeTab === 'today'">
            ...
            <div class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm mb-4">
                <div class="flex items-center justify-between px-4 py-2.5 bg-slate-50 border-b border-slate-100">
                    ...
                    <span class="text-xs text-slate-400">
                        <strong class="text-slate-700 font-bold">{{ filteredTodayQueue.length }}</strong> / {{ todayQueue.length }} shown
                    </span>
                </div>
                <div class="p-3">
            ...
            <div v-else-if="!filteredTodayQueue.length"
                class="bg-white rounded-xl border border-slate-200 py-12 text-center">
                <p class="text-sm font-semibold text-slate-400">No patients match search</p>
            </div>
            ...
            <div v-else class="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm">
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-slate-50">
                        <tr v-for="item in filteredTodayQueue" :key="item.id"
        ...
        <div v-if="activeTab === 'pending'">
            ...
            <div class="bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm mb-4">
                <div class="flex items-center justify-between px-4 py-2.5 bg-slate-50 border-b border-slate-100">
                    ...
                    <button v-if="search || date || visitType !== 'all'"
                        @click="clearFilters"
                ...
                <div class="p-3 space-y-2.5">
                    <div class="flex items-center gap-2">
                        ...
                        <Button size="sm" class="h-8 text-xs gap-1.5"
                            style="background-color:#059669; color:white;" @click="applyFilters">
                    ...
                    <div class="flex items-center gap-1.5 flex-wrap">
                        ...
                        <button v-for="t in [
                            ...
                            @click="visitType = t.value; applyFilters()"
                            class="px-2.5 py-1 text-xs font-semibold rounded-full border transition-all"
                            :style="visitType === t.value
                                ? 'background-color:#059669;color:white;border-color:#059669;box-shadow:0 1px 4px rgba(5,150,105,0.3);'
