...
import { Building2, Printer, ChevronDown, ChevronUp, FileText, Plus, CheckCircle2,
         Banknote, CreditCard, Smartphone, Receipt as ReceiptIcon, DollarSign } from 'lucide-vue-next'
...
function printAllSOA() {
    ...
    <style>
        * { box-sizing:border-box; margin:0; padding:0; }
        ...
        .soa-page:last-child { page-break-after: avoid; }
    </style></head>
    <body>${parts.map(p => `<div class="soa-page">${p}</div>`).join('')}</body></html>`)
    doc.close()
    iframe.onload = () => {
        iframe.contentWindow.focus()
        iframe.contentWindow.print()
    ...
    <AppLayout title="Company Billing">
            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-slate-400 text-xs">Pre-employment invoices billed to referring companies</p>
                    </div>
                </div>
        ...
        <div class="space-y-5">
            ...
            <div class="grid grid-cols-3 gap-4">
                <div class="bg-card rounded-xl border shadow-sm p-4 flex items-center gap-4">
                ...
                <div class="bg-card rounded-xl border shadow-sm p-4 flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-2xl font-black text-slate-800">{{ summary.invoice_count }}</p>
                    </div>
                </div>
                <div class="bg-card rounded-xl border shadow-sm p-4 flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-xs text-slate-500 font-medium">Total Outstanding</p>
                        <p class="text-xl font-black text-red-600">{{ peso(summary.total_balance) }}</p>
                    </div>
                </div>
            </div>
            ...
            <div v-if="companies.length === 0"
                ...
                <p class="text-sm mt-1">Mark invoices as "Company Billing" from the invoice detail page.</p>
            </div>
                    ...
                    <table class="w-full">
                        ...
                        <tfoot>
                            <tr class="bg-slate-100 border-t-2 border-slate-300">
                                <td colspan="4" class="px-5 py-3 text-sm font-bold text-slate-700 text-right">Totals</td>
                ...
                <table style="width:100%;border-collapse:collapse;font-size:11px;">
                    ...
                    <tfoot>
                        <tr class="total-row" style="border-top:2px solid #111;">
                            <td colspan="4" style="padding:8px;font-weight:900;text-align:right;font-size:12px;">TOTAL OUTSTANDING BALANCE</td>
                            <td colspan="3" style="padding:8px;text-align:right;font-weight:900;font-size:14px;color:#dc2626;">{{ peso(c.total_balance) }}</td>
                        </tr>
                    </tfoot>
                </table>
